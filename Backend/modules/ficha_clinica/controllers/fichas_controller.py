...
# controllers/fichas_controller.py (Solo la función update_ficha)
...
def update_ficha(rut_residente):
    ...
    # --- CORRECCIÓN ---
    # 1. Eliminamos el bloque try-except innecesario.
    # 2. Confiamos únicamente en el RUT que viene en la URL (rut_residente).
    # 3. Flasgger y MongoDB solo necesitan el RUT para actualizar.
    
    result = db.fichas.update_one({"rut_residente": rut_residente}, {"$set": data})
    
    if result.matched_count == 0:
        # Aunque es raro, manejamos si no encuentra el documento
        return jsonify({"message": "Ficha no encontrada para el RUT proporcionado."}), 404

    # --- FIN CORRECCIÓN ---
    
    return jsonify({"message": f"{result.modified_count} ficha(s) actualizada(s)"})

# (El resto del archivo, como create_ficha, delete_ficha, y get_ficha, se mantiene intacto)
